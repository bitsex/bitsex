...
#include "zbitsexcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBTSX ought to be enough for anybody." - Bill Gates, 2017
    ui->zBTSXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBTSXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBTSX->setEnabled(false);
        ui->pushButtonMintzBTSX->setToolTip(tr("zBTSX is currently disabled due to maintenance."));

        ui->pushButtonSpendzBTSX->setEnabled(false);
        ui->pushButtonSpendzBTSX->setToolTip(tr("zBTSX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBTSXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBTSX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTSX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBTSX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBTSX in ") +
...
void PrivacyDialog::on_pushButtonSpendzBTSX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTSX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBTSX
        sendzBTSX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBTSX
    sendzBTSX();
}

void PrivacyDialog::on_pushButtonZBitsexControl_clicked()
{
    ZBitsexControlDialog* zBTSXControl = new ZBitsexControlDialog(this);
    zBTSXControl->setModel(walletModel);
    zBTSXControl->exec();
}

void PrivacyDialog::setZBitsexControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBTSXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBTSX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bitsex Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBTSXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBTSX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BITSEX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBTSX</b>";
    ...
    // use mints from zBTSX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBitsexControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitsexControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbitsex selector in case it was used
    ZBitsexControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBTSX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bitsex, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBTSX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBTSX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBTSX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBTSX "));
    ui->labelzBTSXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBTSXSyncStatus->setVisible(fShow);
}
