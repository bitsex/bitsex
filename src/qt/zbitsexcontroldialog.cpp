...
#include "ui_zbitsexcontroldialog.h"
...
std::list<std::string> ZBitsexControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitsexControlDialog::listMints;

ZBitsexControlDialog::ZBitsexControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitsexControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitsexControlDialog::~ZBitsexControlDialog()
{
    delete ui;
}

void ZBitsexControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitsexControlDialog::updateList()
{
...
void ZBitsexControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitsexControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitsex_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitsexControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitsexControlDialog::GetSelectedMints()
{
...
void ZBitsexControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
